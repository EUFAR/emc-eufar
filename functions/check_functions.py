...
def fill_all_fields(self):
    ...
    if self.qv_tabWidget.count() > 0:
        for i in range(self.qv_tabWidget.count()):
            self.qv_tabWidget.tabBar().setTabTextColor(i, QColor(0,0,0))
    ...
    for check_box in all_check_boxes:
        if check_box.objectName()[0:3] == "cl_" and check_box.isChecked():
            ...
            all_checks_kw = 1
    if all_checks_cl == 0:
        self.tabWidget.tabBar().setTabTextColor(1, QColor(200,0,0))
    if all_checks_kw == 0:
        self.tabWidget.tabBar().setTabTextColor(2, QColor(200,0,0))
    
    # check : checkboxes in QV section:
    j = 0
    for i in range(self.qv_tabWidget.count()):
        if "Atmospheric" in self.qv_tabWidget.tabText(i):
            all_check_boxes = self.qv_tabWidget.widget(i).findChildren(QCheckBox)
            checks_qv = 0
            for check_box in all_check_boxes:
            ...
            if checks_qv == 0:
                ...
                self.tabWidget.tabBar().setTabTextColor(6, QColor(200,0,0))
                self.qv_tabWidget.tabBar().setTabTextColor(i, QColor(200,0,0))
                all_checks_qv = 1
            j += 1
    ...
    for sublist in mandatory_radio_list:
        ...
        if not widget:
            ...
            self.tabWidget.tabBar().setTabTextColor(6, QColor(200,0,0))
            self.qv_tabWidget.tabBar().setTabTextColor(sublist[2], QColor(200,0,0))
        ...
        if not sublist[0].isHidden():
            if sublist[0].toPlainText() == "":
                if "qv_" in sublist[0].objectName():
                    ...
                    sublist[1].setStyleSheet("color: rgb(200,0,0)")
                    self.tabWidget.tabBar().setTabTextColor(6, QColor(200,0,0))
                    self.qv_tabWidget.tabBar().setTabTextColor(sublist[3], QColor(200,0,0))
                else:
                    all_texts = 0
                    sublist[1].setStyleSheet("color: rgb(200,0,0)")
                ...
                if "qv_" in sublist[0].objectName():
                    if not check_field_correct_area(sublist[0], sublist[2]):
                        ...
                        if self.tabWidget.tabBar().tabTextColor(6).red() != 200:
                            self.tabWidget.tabBar().setTabTextColor(6, QColor(0,0,200))
                        if self.qv_tabWidget.tabBar().tabTextColor(sublist[3]).red() != 200:
                            self.qv_tabWidget.tabBar().setTabTextColor(sublist[3], QColor(0,0,200))
                else:
                    if not check_field_correct_area(sublist[0], sublist[2]):
                        all_texts = 0
                        sublist[1].setStyleSheet("color: rgb(0,0,200)")
    ...
    if not self.aircraft_list :
        ...
        self.tabWidget.tabBar().setTabTextColor(3, QColor(200,0,0))
        all_aircraft = 0
    if not self.instModel_list:
            ...
            if date > current_date:
                if "qv_" in sublist[0].objectName():
                    ...
                    if self.qv_tabWidget.tabBar().tabTextColor(sublist[2]).red() != 200:
                        self.qv_tabWidget.tabBar().setTabTextColor(sublist[2], QColor(0,0,200))   
                else :
                    all_dates = 0
                    sublist[1].setStyleSheet("color: rgb(0,0,200)")
...
def check_field_correct_area(widget, field_format):
    ...
    if field_format == "text":
        try:
            tmp = float(widget.toPlainText())  # @UnusedVariable
            result = False
        except ValueError:
            result = True
    elif field_format == "float":
